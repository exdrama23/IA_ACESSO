...
function IAVisual() {
  ...
  useFrame((state) => {
    ...
    targetMouse.current.z = 1.0; 

    currentMouse.current.lerp(targetMouse.current, 0.1);
    material.current.uniforms.uMouse.value.copy(currentMouse.current);

    const swayX = Math.sin(time * 0.5) * 0.05;
    const swayY = Math.cos(time * 0.3) * 0.1;

    const targetRotationX = (-state.pointer.y * 0.5) + swayX;
    const targetRotationY = (state.pointer.x * 0.5) + swayY;
