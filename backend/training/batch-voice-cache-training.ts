...
import { gerarAudio } from '../src/ai/tts';
...
interface RespostaUnica {
  resposta: string;
  perguntas: string[];
  perguntasValidas: string[];
  perguntasRejeitadas: string[];
  categoria: string;
  mediaRelevancia: number;
}
 ...
 */
function isPerguntaValida(pergunta: string): boolean {
 ...
 */
function calcularRelevancia(pergunta: string, resposta: string): number {
  const similarity = compareTwoStrings(pergunta.toLowerCase(), resposta.toLowerCase());
 ...
 */
function extrairRespostasUnicas(): RespostaUnica[] {
  const respostasMap = new Map<string, RespostaUnica>();

  Object.entries(faq).forEach(([_chave, itens]) => {
    itens.forEach(item => {
 ...
 */
async function gerarAudioComRetry(
  texto: string,
  tentativasRestantes: number = 3
): Promise<string | null> {
  try {
    const audioUrl = await gerarAudio(texto, true); // forçar geração
...
async function executarTreino() {
  try {
    ...
    const jáExistentes = await prisma.voiceCache.findMany({
