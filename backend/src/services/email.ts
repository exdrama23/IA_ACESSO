...
export async function sendIntegrationVerificationEmail(
  email: string,
  code: string,
  service: string,
  userName?: string | null,
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
    ...
    <tr>
      <td style="padding: 35px 30px;">
        ...
        <p style="font-size: 16px; margin: 0 0 30px 0; color: #555555; line-height: 1.5;">
          Você está tentando alterar a configuração de credenciais para <strong>${serviceName}</strong>. Para prosseguir, confirme sua identidade usando o código abaixo.
        </p>
        ...
        <div style="background-color: #fff0f5; border-left: 1px solid #fff0f5; padding: 15px 20px; border-radius: 0 8px 8px 0; margin-bottom: 25px;">
          <p style="margin: 0; font-size: 14px; color: #444444; line-height: 1.6;">
            <strong style="color: #e50b5e;">Segurança:</strong> Nunca compartilhe este código com ninguém. A ACESSO.NET.BR nunca solicitará este código por telefone ou mensagem.
          </p>
        </div>
    ...
    await transporter.sendMail({
      from: process.env.EMAIL_USER,
      to: email,
      subject: `Verificação de Segurança - ${serviceName} - ACESSO.NET.BR`,
      html: htmlContent,
      text: `Olá, ${displayName}. Seu código de verificação de segurança para alterar credenciais do ${serviceName}: ${code}\nEste código expira em 15 minutos.`,
    });

    console.log(`[EMAIL] Código de verificação de integração enviado com sucesso para: ${email}`);
  } catch (error) {
    console.error('[EMAIL] Erro ao enviar email de verificação de integração:', error);
    throw new Error(
